...
import { Action, BaseCellar, Module, UserEventAction } from '../app.module';
...
import { AllocationEventComponent } from './allocation-event/allocation-event.component';
...
export class AllocationComponent implements OnInit {
  ...
  ngOnInit(): void { 
    this.authService.getUserProfile().subscribe((profile) => {
      ...
      this.allocation.get(this.profile.id).subscribe((results) => {
  ...
  public get ActiveAllocationCount() {
    return this.allocations
      ? this.allocations.filter((a) => a.status == 'Active').length
      : 0;
  }
      ...
      this.allocations
        ...
        .map((a) => {
          const upcomingEvents = a.events?.filter((e) => {
            const currentDate = getMonthFromString(
  ...
  onAllocationAction(action: Action) {
    this.userSelectionEvent = new UserEventAction(
      Action[action],
      Module.Allocation
    );
  }
